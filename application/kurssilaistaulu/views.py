...
def kurssilaistaulu_index():
    return render_template("kurssilaistaulu/kurssilaistauluform.html", form = StudentForm(), 
                           course_students=User.studentto_course(), courses = Course.query.all())
...
def student_create():
    ...
    if User.query.filter(User.name == form.user.data).count() == 0:
        return render_template("/kurssilaistaulu/kurssilaistauluform.html", form = StudentForm(),course_students=User.studentto_course(), courses = Course.query.all(), error = "Tämän nimistä kurssilaista ei löytynyt") 

    if Course.query.filter(Course.name == form.course.data).count() == 0:
        return render_template("/kurssilaistaulu/kurssilaistauluform.html", form = StudentForm(), course_students=User.studentto_course(), courses = Course.query.all(), error = "Tämän nimistä kurssia ei löytynyt")     
